...
import { TOOL_SPECS, TIERS } from './registry.js';
...
// --- integration ------------------------------------------------------------
//
// The integration pages in the viewer show the real starter tiering and the
// real command list. Both are read off disk rather than retyped into JSX,
// because config copy that drifts from the config is worse than no copy at
// all: someone pastes it, it does not work, and the tool has lied to them
// once before it ever ran.
...
app.get('/api/integration', (req, res) => {
  const cli = readRepoFile('cli', 'tracer.mjs') || '';
  const usage = (cli.match(/const USAGE = `([\s\S]*?)`;/) || [])[1] || null;
  res.json({
    // Repo-local, deliberately: Tracer is not published to npm, so `npx tracer`
    // resolves through the cloned repo's own bin entry and nowhere else.
    install: 'npm install',
    cliUsage: usage,
    mcp: {
      tiersPath: 'adapters/mcp/tiers.json',
      tiers: readRepoFile('adapters', 'mcp', 'tiers.json'),
      binPath: 'adapters/mcp/bin/tracer-proxy.mjs',
    },
    extension: {
      distPath: 'adapters/browser/dist',
      tiersPath: 'adapters/browser/src/tiers.js',
    },
  });
});
...
app.get('/api/meta', (req, res) => {
  res.json({
    ...
    attacks: ATTACKS.map((a) => ({
      ...
      note: a.note || null,
      // Where the class comes from. Surfaced in the Viewer because an attack the
      // user cannot trace to a published finding is an attack they have to take
      // our word for -- see SOURCES in shared/attacks.js.
...
// Evidence from the proof-of-life demo: the proxy in front of two real MCP
// servers, refusing a real write. Regenerated by `npx tracer demo-proxy --both`.
// Served rather than inlined so the page shows the artifact from the last real
// run rather than a screenshot of one.
app.get('/api/proxy-evidence', (req, res) => {
  const path = join(ROOT, 'demo', 'evidence', 'evidence.json');
  if (!existsSync(path)) {
    return res.status(404).json({ error: 'no demo evidence yet -- run `npx tracer demo-proxy --both`' });
  }
  res.type('application/json').send(readFileSync(path, 'utf8'));
});
