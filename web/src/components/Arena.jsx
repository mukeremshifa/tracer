        ...
        <div className="stack">
          ...
          {board && board.entries.length > 0 && (
            <div className="panel">
              ...
              {/*
                Rows labelled "tracer team (seed)" are ours, generated by
                scripts/seed-arena.mjs. Saying so costs nothing and inventing
                submitter names would be fabricating an audience -- which is
                exactly the move this project argues against everywhere else.
              */}
              {board.entries.some((e) => /seed/.test(e.handle || '')) && (
                <p className="small faint" style={{ margin: '0 0 14px' }}>
                  Rows marked <span className="mono">tracer team (seed)</span> are ours &mdash; real runs
                  through this same pipeline, checked in so the board is not empty before anyone has tried
                  it. Everything else came from a visitor.
                </p>
              )}
