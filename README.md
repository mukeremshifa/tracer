...
npx tracer demo-proxy --both         # the proxy vs. two real MCP servers, both halves
npx tracer sandbox                   # the demo
```

The sandbox is legitimate and it is not the product. Once a real adapter blocks a real tool call, the
sandbox is a demo of something real.
...
cli/tracer.mjs           test | proxy | demo-proxy | log | sandbox | extension
...
demo/                    PROOF OF LIFE — the proxy vs. two real MCP servers
  ...
  evidence/              both transcripts, regenerated by `tracer demo-proxy --both`
